...
public class InputParser {
	...
	Boolean validateOrbitSpeedFormat(String orbitTrafficSpeed, String orbit) {
		...
		if (trafficString.size()==6 && trafficString.get(0).equals(orbit)
				&& (trafficString.get(1) + trafficString.get(2) + trafficString.get(3)).equals("trafficspeedis")
				&& trafficString.get(4).matches("[+-]?([0-9]*[.])?[0-9]+") && trafficString.get(5).equals("megamiles/hour")) {
			userFormat = false;
		} else {
			userFormat = true;
		}
